...
app.post("/api/auth/login", async (req: Request, res: Response) => {
  ...
  try {
    const result = await pool.query("SELECT id, email, encrypted_password FROM auth.users WHERE email = $1", [email]);
...
app.post("/api/auth/check-password", authenticateJWT, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const result = await pool.query("SELECT encrypted_password FROM auth.users WHERE id = $1", [userId]);
    if (result.rows.length === 0) {
...
// Change password
app.post("/api/auth/change-password", authenticateJWT, async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
...
app.get("/api/auth/settings", authenticateJWT, async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(200).json({ pin_enabled: false, pin_hash: null, display_name: null });
    }
...
app.post("/api/auth/settings", authenticateJWT, async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  const { pin_enabled, pin_hash, display_name } = req.body;

  try {
    // Check if settings exist to decide update or insert
    const existCheck = await pool.query("SELECT id FROM public.user_settings WHERE user_id = $1", [userId]);
    ...
    if (existCheck.rows.length > 0) {
      ...
      params = [userId, pin_enabled || false, pin_hash || null, display_name || null];
    }
...
app.get("/api/auth/salt", authenticateJWT, async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  try {
    const result = await pool.query("SELECT encryption_salt FROM public.user_encryption_keys WHERE user_id = $1", [
      userId,
    ]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "Encryption salt not found for user" });
    }
...
app.post("/api/auth/salt", authenticateJWT, async (req: AuthRequest, res: Response) => {
  const userId = req.user?.id;
  const { salt } = req.body;
  ...
  try {
    const check = await pool.query("SELECT user_id FROM public.user_encryption_keys WHERE user_id = $1", [userId]);
